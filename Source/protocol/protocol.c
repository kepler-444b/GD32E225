...
static void app_proto_hi(usart0_rx_buf_t *buf)
{
    ...
    if (buf->buffer[0] == 'O' && buf->buffer[1] == 'K') { // 先收到"OK,标记为"掉线"
        ...
        if (second_quote >= end_ptr)
            return; // 未找到第二个双引号

        size_t hex_len = second_quote - (first_quote + 1);
        if (hex_len == 0 || hex_len % 2 != 0 || hex_len > sizeof(my_valid_data.data))
            return; // 必须为偶数
...
static void app_proto_lhw(usart0_rx_buf_t *buf)
{
    ...
    memset(&my_valid_data, 0, sizeof(my_valid_data));
    // 检查前导码
    if (buf->buffer[0] != 0x1b && buf->buffer[7] != 0x0a)
        return;
...
static uint8_t combined_buf[MAX_BUF_SIZE]; // 静态缓冲区，用于存储合并的数据
static uint16_t combined_len = 0;          // 当前缓冲区的有效数据长度
static uint32_t last_receive_time = 0;     // 上次数据接收的时间

// usart0 接收到数据首先回调在这里处理
static void app_proto_check(usart0_rx_buf_t *buf)
{
#if 1
    // APP_UappsMsgProcessing(buf->buffer, buf->length);
    // APP_PRINTF_BUF("buf", buf->buffer, buf->length);
    MCU_UartReceive(buf->buffer, buf->length);
    // APP_UappsMsgProcessing(buf->buffer, buf->length);
#else
    // 在恢复出厂设置的命令,不是完整的一包数据,在此定义20ms的接收窗口,组成一包完整的数据
    uint32_t current_time = app_timer_get_ticks();

    // 检查是否超时，如果超时则处理已缓冲的数据
    if (current_time - last_receive_time > TIMEOUT_THRESHOLD) {
        if (combined_len > 0) {
            // MCU_UartReceive(combined_buf, combined_len);
            APP_UappsMsgProcessing(combined_buf, combined_len);
            combined_len = 0;
        }
    }

    if (combined_len + buf->length <= MAX_BUF_SIZE) { // 将当前收到的数据追加到缓冲区
        memcpy(&combined_buf[combined_len], buf->buffer, buf->length);
        combined_len += buf->length;
    } else {
        combined_len = 0; // 清空缓冲区
    }
