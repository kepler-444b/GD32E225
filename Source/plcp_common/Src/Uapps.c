...
void uapps_option_nibble(U32 value, uint8_t *nibble)
{
    if (value < 13) {
    ...
    for (i = 0; i < msg->num_opts - 1; i++) {
        for (j = 0; j < msg->num_opts - 1 - i; j++) {
            if (msg->options[j].opt_code > msg->options[j + 1].opt_code) {
                ...
                memcpy(&msg->options[j], &msg->options[j + 1], sizeof(UappsOption));
                memcpy(&msg->options[j + 1], &tempOtp, sizeof(UappsOption));
            }
        }
    }
}
...
/*--------------------------------6-----------------------------
函数名称：APP_StrToHex
函数功能：将字符串转为十六进制的uint8_t数组。
输入参数：inputStr--待转换的字符串指针
         inputStrLen--待转换的字符串长度,
输出参数： resultHexArray - 转换后的十六进制的uint8_t数组指针
返回值：转换后的十六进制的uint8_t数组的长度
备 注：
---------------------------------------------------------------*/
...
UappsMessage *UappsBuildReq(char *rsl, uint8_t type, uint8_t hdrCode, char *payload, U32 len)
{
