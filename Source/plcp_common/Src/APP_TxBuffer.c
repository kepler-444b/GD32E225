...
static u16 APP_TxBufferDataParamArrayIndex = 0;
#else
/**********************************静态变量声明 开始**********************************/
static uint8_t APP_TxBuffer[APP_TX_BUFFER_DATA_PARAM_ARRAY_SIZE][APP_TX_BUFFER_SIZE];
static u16 APP_TxBufferDataParamArrayIndex = 0;
...
static u16 APP_TxBufferHead = 0;
static u16 APP_TxBufferTail = 0;
#endif
 ...
 * 4、     u16 APP_TxBuffer_GetFirstMsg(u16 outputDataMaxLen, uint8_t* outputData)
 * 5、     uint8_t APP_TxBuffer_IsFull()
 * 6、     uint8_t APP_TxBuffer_IsEmpty()
 * 7、     uint8_t APP_TxBuffer_CanAddNewMsg(u16 inputDataLen)
...
uint8_t APP_TxBuffer_Add(APP_DataTxTask *inputDataParam)
{
    ...
    if (addFailCount > 3) {
        addFailCount = 0;
        APP_TxBuffer_init();
        LOGINFO0(UART_DEBUG, "tx full reset\n");
    }
    ...
    return 1;
#else
    uint16_t nextTail = (APP_TxBufferTail + 1) % APP_TX_BUFFER_DATA_PARAM_ARRAY_SIZE;
    if (nextTail == APP_TxBufferHead || inputDataParam->nsduLength > APP_TX_BUFFER_SIZE) { // 队列满或者数据过大
        if (nextTail == APP_TxBufferHead) {
            APP_PRINTF("queue full\n");
        } else {
            APP_PRINTF("APP_TX_BUFFER_SIZE too long\n");
        }

        return 0;
    }
    memcpy(APP_TxBuffer[APP_TxBufferTail], inputDataParam->nsdu, inputDataParam->nsduLength);
    memcpy(&APP_TxBufferDataParamArray[APP_TxBufferTail], inputDataParam, sizeof(APP_DataTxTask));
    APP_TxBufferDataParamArray[APP_TxBufferTail].nsdu = APP_TxBuffer[APP_TxBufferTail];
    APP_TxBufferTail = nextTail;
    return 1;
#endif
}
...
uint8_t APP_TxBuffer_DeleteFirstMsg(void)
{
    ...
    return 1;
#else
    if (APP_TxBufferHead == APP_TxBufferTail) {
        return 0;
    }
    APP_TxBufferHead = (APP_TxBufferHead + 1) % APP_TX_BUFFER_DATA_PARAM_ARRAY_SIZE;
    return 1;
#endif
}
...
uint8_t APP_TxBuffer_GetFirstMsgDataParameters(APP_DataTxTask *firstMsgDataParam)
{
    ...
    memcpy(firstMsgDataParam, &APP_TxBufferDataParamArray[APP_TxBufferHead], sizeof(APP_DataTxTask));
    firstMsgDataParam->nsdu = APP_TxBuffer[APP_TxBufferHead];
    return 1;
...
uint8_t *APP_TxBuffer_GetFirstMsgPointer(void)
{
    ...
    if (APP_TxBufferHead == APP_TxBufferTail)
        return NULL;
    return APP_TxBuffer[APP_TxBufferHead];
#endif
}
...
#else
    return ((APP_TxBufferTail + 1) % APP_TX_BUFFER_DATA_PARAM_ARRAY_SIZE) == APP_TxBufferHead;
#endif
}
...
#else
    return APP_TxBufferHead == APP_TxBufferTail;
#endif
}
...
#else
    if (APP_TxBufferTail >= APP_TxBufferHead)
        return APP_TxBufferTail - APP_TxBufferHead;
    else
